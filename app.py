...
DEFAULT_ALLOWED_ORIGINS = [
    ...
    "https://famous-madeleine-c5f449.netlify.app",
    "http://localhost:5500",
    "http://127.0.0.1:5500",
    "http://localhost:8000",
    "http://127.0.0.1:8000",
...
def _pg_sql(sql: str) -> str:
    out = str(sql)
    out = re.sub(r"^\s*BEGIN\s+IMMEDIATE\s*;?\s*$", "BEGIN", out, flags=re.IGNORECASE)
    out = out.replace("INTEGER PRIMARY KEY AUTOINCREMENT", "SERIAL PRIMARY KEY")
    out = re.sub(r"\bdatetime\(([^)]+)\)", r"\1", out, flags=re.IGNORECASE)
    out = re.sub(
    ...
    out = out.replace("?", "%s")
    if re.search(r"\bINSERT\s+INTO\b", out, flags=re.IGNORECASE) and "OR IGNORE" not in out.upper():
        original = str(sql).upper()
        if "INSERT OR IGNORE INTO" in original and "ON CONFLICT" not in out.upper():
            out = out.rstrip().rstrip(";") + " ON CONFLICT DO NOTHING"
    return out


class PgCursor:
    RETURNING_ID_TABLES = {"orders", "order_tickets", "inventory", "queue_tickets", "queue_line_bindings"}
    ...
    def execute(self, sql, params=None):
        text = _pg_sql(sql)
        should_return_id = self._should_return_id(text)
        if should_return_id:
    ...
    def _should_return_id(self, sql: str) -> bool:
        ...
        return bool(match and match.group(1).lower() in self.RETURNING_ID_TABLES)
